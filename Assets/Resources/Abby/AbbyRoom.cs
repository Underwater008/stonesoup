...
public class AbbyRoom : Room
{
    public GameObject abbyWallPrefab;
    ...
    public override void fillRoom(LevelGenerator ourGenerator, ExitConstraint requiredExits)
    {
        ...
        _tileGrid = new AbbyWall[gridWidth, gridHeight];
    ...
    void MakeOtherTiles()
    {
        ...
        for (int x = 0; x < gridWidth; x++)
            for (int y = 0; y < gridHeight; y++)
            {
                        ...
                        if (y >= alcloveYMin && y <= alcloveYMax)
                        {
                            ...
                            if (hasItem && !itemSpawned)
                            {
                                if (Random.Range(0f, 1f) < 1f)
                                {
                                    ...
                                    // _tileGrid[x, y] = AbbyTile.spawnAbbyTile(omnispherePrefab, transform, x, y);
                                    itemSpawned = true;
                                    continue;
                                }
                            }
                ...
                //     {
                //         _tileGrid[x, y].remove();
                //         _tileGrid[x, y] = AbbyTile.spawnAbbyTile(localTilePrefabs[1], this.transform, x, y);
                //     }
                // }
        ...
        for (int x = 0; x < gridWidth; x++)
            for (int y = 0; y < gridHeight; y++)
            {
                ...
                if (_boolGrid[x, y])
                {
                    ...
                    _tileGrid[x, y] = AbbyTile.spawnAbbyTile(abbyWallPrefab, transform, x, y);
    ...
    public override Room createRoom(ExitConstraint requiredExits)
    {
        ...
        return roomObj.GetComponent<AbbyRoom>();
    }
    void CreateCave()
    {
